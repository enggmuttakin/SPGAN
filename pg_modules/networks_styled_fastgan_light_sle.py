...
from pg_modules.blocks import (InitLayer, UpBlockBig, UpBlockBigSP, UpBlockBigCond, UpBlockSmall, UpBlockSmallCond, SEBlock, SEBlockSP, conv2d)
...
class FastganSynthesis(nn.Module):
    def __init__(self, ngf=128, z_dim=256, w_dim=256, nc=3, img_resolution=256, lite=False):
        ...
        self.se_64  = SEBlockSP(nfc[4], nfc[64])
        self.se_128 = SEBlockSP(nfc[8], nfc[128])
        self.se_256 = SEBlockSP(nfc[16], nfc[256])

        self.to_big = conv2d(nfc[img_resolution], nc, 3, 1, 1, bias=True)

        if img_resolution > 256:
            self.feat_512 = UpBlock(nfc[256], nfc[512])
            self.se_512 = SEBlockSP(nfc[32], nfc[512])
